...
function Contact() {
  ...
  const contact: ContactType[] = [
    {icon: 'logo-github', text: 'GitHub', link: 'https://github.com/faramir07'},
    {icon: 'logo-linkedin', text: 'Linkedin', link: 'https://www.linkedin.com/in/miguel-linares-gamez'},
    {icon: 'mail', text: 'Malg20012002@gmail.com', link: 'mailto:malg20012002@gmail.com'},
  ]
  ...
  const sendEmail = (e: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if ((input.user_name = '') && (input.user_email = '') && (input.message = '')) {
      ...
      emailjs.sendForm('service_htr9d3m', 'template_6jomwql', target , '_sBLAnjn5TWnr6qGm')
      .then(response => {
      ...
      .catch(error => {
        alert.fire({
          ...
          html: <div><span>Puedes con tactarme por </span><a href={contact[1].link}><IonIcon name={contact[1].icon}></IonIcon></a></div>,
        })        
      })
    }
  }
  ...
  return (
    <section id='contact' className="py-10 px-3 text-white">
      ...
      <div className="mt-16 flex md:flex-row flex-col gap-6 max-w-5xl bg-gray-800 p-6 rounded-lg mx-auto">
        <form onSubmit={sendEmail} className="flex flex-col flex-1 gap-6" action="">
          <input type="text" defaultValue="" name="user_name" placeholder="Tu Nombre" onChange={handleChange} required />
          {error.user_name? <span className='text-red-600'>{error.user_name}</span> : null}
          <input type="email" defaultValue="" name="user_email" placeholder="Tu Correo" onChange={handleChange} required/>
          {error.user_email? <span className='text-red-600'>{error.user_email}</span> : null}
          <textarea placeholder="Tu Mensage" defaultValue="" name="message" rows={10} onChange={handleChange} required></textarea>
